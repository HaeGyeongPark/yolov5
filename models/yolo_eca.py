...
class ECA(nn.Module):
    def __init__(self, c1, c2, k=3, s=1): 
        super().__init__()
        self.conv = nn.Conv2d(c1, c2, k, s, autopad(k), bias=True)
...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=(), inplace=True):
        ...
        self.m = nn.ModuleList(ECA(x, self.no * self.na, 1) for x in ch)   # output conv
...
class Segment(Detect):
    ...
    def __init__(self, nc=80, anchors=(), nm=32, npr=256, ch=(), inplace=True):
        ...
        self.m = nn.ModuleList(ECA(x, self.no * self.na, 1) for x in ch) 
